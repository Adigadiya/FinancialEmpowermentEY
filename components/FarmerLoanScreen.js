...
const FarmerLoanScreen = () => {
    ...
    const fetchFarmerLoans = async () => {
        try {
            ...
            const response = await axios.get("https://dhansangini-backend.onrender.com/api/farmer-loans");
    
            console.log("API Response:", response.data);
    
            if (!response.data || response.data.length === 0) {
                console.warn("⚠️ No farmer loans found!");
                setLoans([]);
                setLoading(false);
                return;
            }
...
export default FarmerLoanScreen;
